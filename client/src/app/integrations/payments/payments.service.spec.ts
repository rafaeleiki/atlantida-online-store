...
import {async} from "q";
import {
  BoletoTransactionRequest, OrangoPagBoletoTransactionRequest, OrangoPagConsultTransactionListRequest,
    ...
    async(
      inject([PaymentsService],
        ...
        service.postDebit(dtr)
          .then(response => expect(
            response.message === 'Transação via debito criada com sucesso').toBeTruthy());
        })));
